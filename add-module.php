...
<?php
if (isset($_GET['submit_module'])) {
  $course_id = $_GET['course_name'];
  $module_name = $_GET['module_name'];
  
  // Check that the course_id exists in the courses table
  $query = "SELECT * FROM registration.courses WHERE course_id = $course_id";
  $result = $db->query($query);
  if ($result->num_rows > 0) {
      // Insert the new module record
      $sql = "INSERT INTO registration.modules (module_name, course_id, id_user) VALUES ('$module_name', $course_id, '1')";
      ...
      echo "Error: Invalid course selected";
  }
}
